    ...
    for feed in Feed.query.all():
        try:
            ...
            parsed_feed = feedparser.parse(
                feed.uri, etag=feed.etag, modified=feed.modified
            )
            ...
            for entry in sorted(parsed_feed.entries, key=lambda e: e.update_time):
                ...
                if last_publish_time is None or last_publish_time < publish_time:
                    ...
                    if feed.is_aggregator:
                        ...
                        try:
                            article_uri = requests.head(
                                article_uri, allow_redirects=True
                            ).url
                        except:
                            continue
...
def tasks_reschedule():
    ...
    for article in Article.query.filter(
        (Article.status == "Q") & (Article.update_time < hour_ago)
    ):
        article.retries += 1
...
def tasks_next():
    article = (
        Article.query.filter(Article.status == "N").order_by(func.random()).first()
    )
