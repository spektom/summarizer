...
local_file_header = [("signature", "uint32"), ("min_version", "uint16"),
                     ("general_flag", "uint16"), ("compression", "uint16"),
                     ("lastmod_time", "uint16"), ("lastmod_date", "uint16"),
                     ("crc32", "uint32"), ("compressed_size", "uint32"),
    ...
    def read_struct(self, format, offset=None):
        ...
        for (name, _) in iter(format):
            ...
            if not name in string_fields:
                ...
                member_data = self.readAt(self.offset,
                                          retstruct.__getattr__(member_desc))
        ...
        assert_true(out_data == data,
                    "Serialization fail %d !=%d" % (len(out_data), len(data)))
        return retstruct
...
def optimizejar(jar, outjar, inlog=None):
    ...
    if inlog is not None:
        ...
        out_offset = dirend.cdir_offset + dirend.cdir_size + size_of(
            cdir_end) - total_stripped
    ...
    for entry in central_directory:
        ...
        assert_true(jarfile.filename == entry.filename,
        ...
        # January 1st, 2010
        entry.lastmod_date = jarfile.lastmod_date = (
            (2010 - 1980) << 9) | (1 << 5) | 1
        ...
        assert_true(
            len(entry_data) != expected_len,
            "%s entry size - expected:%d got:%d" %
            (entry.filename, len(entry_data), expected_len))
        written_count += 1
    ...
    if dups_found > 0:
        print("WARNING: Found %d duplicate files taking %d bytes" %
              (dups_found, dupe_bytes))

    dirend.cdir_size = len(cdir_data)
    dirend.disk_entries = dirend.cdir_entries
    dirend_data = dirend.pack()
    assert_true(
        size_of(cdir_end) == len(dirend_data),
        "Failed to serialize directory end correctly. Serialized size;%d, expected:%d"
        % (len(dirend_data), size_of(cdir_end)))
    ...
    print("%s %d/%d in %s" %
          (("Ordered" if inlog is not None else "Deoptimized"),
...
if len(sys.argv) != 5:
    print(
        "Usage: --optimize|--deoptimize %s JAR_LOG_DIR IN_JAR_DIR OUT_JAR_DIR"
        % sys.argv[0])
    exit(1)
